        ...
        for number in range(1, 7):
            ...
            for item in range(5):
                if x_casinos[number]['banknotes'][item] == 0:
                    break
                print(x_casinos[number]['banknotes'][item], end=' ')
                msg += ' LVD' + str(x_casinos[number]['banknotes'][item])
            print('\n玩家在赌场{0}的置放情况是：'.format(number))
            msg += '\n玩家在赌场{0}的置放情况是：\n'.format(number)
            for item in range(1, x_player_number + 1):
                print('P{0}放了DIC{1}；'.format(item, x_casinos[number]['player' + str(item)]), end='\t')
                msg += 'P{0}放了DIC{1}；\t'.format(item, x_casinos[number]['player' + str(item)])
            print()
            msg += '>\n'
        return msg
